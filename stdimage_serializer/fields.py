...
class StdImageField(serializers.ImageField):
    ...
    def get_variations_urls(self, obj):
        ...
        # A lot of ifs going araound,
        # first check if it has the field variations
        if hasattr(field, 'variations'):
            # Get the variations
            variations = field.variations
            # Go through the variations dict
            for key, attr in variations.items():
                # Just to be sure if the stdimage
                # object has it stored in the obj
                if hasattr(obj, key):
                    ...
                    if url:
                        # store it, with the name of the
                        # variation type into our return object
